...
class NeuralNetwork():
    ...
    def train(self, x1, x2, y):
        ...
        _scalar_part = -(y - activated_output) * output_layer_activation_derivative(output)
        _vector_part = self.hidden_to_output_weights * rectified_linear_unit_derivative(hidden_layer_weighted_input).transpose()
        bias_gradients = _scalar_part * _vector_part

        hidden_to_output_weight_gradients = -(y - activated_output) * output_layer_activation_derivative(output) * hidden_layer_activation

        input_to_hidden_weight_gradients = \
           -(y - activated_output) * output_layer_activation_derivative(output) * self.hidden_to_output_weights.transpose() * rectified_linear_unit_derivative(hidden_layer_weighted_input) @ input_values.transpose()

        # Use gradients to adjust weights and biases using gradient descent
        self.biases -= self.learning_rate * bias_gradients.transpose()
        self.input_to_hidden_weights -= self.learning_rate * input_to_hidden_weight_gradients
        self.hidden_to_output_weights -= self.learning_rate * hidden_to_output_weight_gradients.transpose()
